...
- **`protocol_version`** — the version of this protocol used (currently `"v7"`)
- For Suite G results, use `protocol_version: "v7"`. Existing Suite A-F results produced under v5 remain valid — Suite G is additive and does not alter any v5 test definitions. Mixed-version result sets (v5 for Suites A-F, v7 for Suite G) are expected and do not require re-evaluation of existing results. Results produced under v6 (before cleaned case export and G-FNM-4 reframing) should be re-evaluated under v7.
