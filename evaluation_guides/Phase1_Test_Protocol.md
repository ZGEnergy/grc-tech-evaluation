...
- **`protocol_version`** — the version of this protocol used (currently `"v6"`)
- For Suite G results, use `protocol_version: "v6"`. Existing Suite A-F results produced under v5 remain valid — Suite G is additive and does not alter any v5 test definitions. Mixed-version result sets (v5 for Suites A-F, v6 for Suite G) are expected and do not require re-evaluation of existing results.
