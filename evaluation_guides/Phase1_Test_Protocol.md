...
**If PSS/E parsing fails** (the tool cannot ingest the intermediate CSV tables at `FNM_PATH`): record a `blocking` `api-friction` observation, set G-FNM-1 `status: fail` with `failure_reason: psse_parse_error`. Proceed to G-FNM-3, G-FNM-4, and G-FNM-5 using the pre-cleaned MATPOWER fallback (`data/fnm/reference/cleaned/fnm_main_island.mat`). Block only G-FNM-2 (which requires intermediate CSV tables for field-level comparison).
