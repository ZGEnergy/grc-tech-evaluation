...
| E-8 | Review/approval concentration | Using `gh api`, enumerate unique PR reviewers/approvers over last 24 months. Compute reviewer concentration (% of reviews by top reviewer). If project has <10 PRs with formal reviews in 24 months, record "no formal review process" as the finding — score as equivalent to reviewer bus factor = 1 (single-gatekeeper risk). | Unique reviewer count, top-reviewer percentage, reviewer bus factor assessment. For non-PR workflows: explicit "no formal review process" finding. |
...
| G-FNM-3 | DCPF verification | Cleaned MATPOWER case (`data/fnm/reference/cleaned/fnm_main_island.mat`), DCPF reference solution | Load the pre-cleaned MATPOWER case (27,862-bus main island with all data fixes pre-applied per `summary_cleaning.json`). Ingest into the tool's data model. Solve DCPF. Extract bus voltage angles and branch active power flows. Load the DCPF reference solution from `data/fnm/reference/dcpf/`. Compute aggregate deviation metrics as defined in `pass_conditions.json` under the `dcpf` key: (a) fraction of buses with VA deviation within tolerance, (b) fraction of in-service branches with P deviation within tolerance. Check against aggregate pass thresholds and hard-fail thresholds. | Pass if all aggregate thresholds are met and no hard-fail condition is triggered, per the `dcpf` section of `data/fnm/reference/pass_conditions.json`. Buses and branches that exceed the aggregate tolerance but are classified as known outlier causes (per the outlier classification rules in pass_conditions.json) are reported as classified outliers, not unqualified failures. If the tool cannot solve DCPF on the FNM at all (solver failure, out of memory, or timeout after 10 minutes), record the failure mode. If failure is due to scale (tool works on MEDIUM but not LARGE), attribute to Scalability. If failure is due to data model issues (missing topology, incorrect parameters), attribute to Expressiveness. | `data/fnm/reference/cleaned/fnm_main_island.mat` (cleaned case), `data/fnm/reference/pass_conditions.json` (thresholds), `data/fnm/reference/dcpf/` (reference angles and flows) |
| G-FNM-4 | ACPF convergence | Cleaned MATPOWER case (`data/fnm/reference/cleaned/fnm_main_island.mat`), ACPF failure analysis | Load the pre-cleaned MATPOWER case. Attempt ACPF solve. **No reference solution exists** -- MATPOWER 8.1 fails on all solver variants (voltage collapse at ~30% load; see `data/fnm/reference/acpf/summary_acpf.json`). This test assesses **solver robustness**: can the tool converge where MATPOWER cannot? Record: (a) whether ACPF converges, (b) solver algorithm used, (c) convergence residual and iteration count, (d) if converged, VM/VA statistics (min, max, mean) and total losses. If two or more tools converge, compare their solutions cross-tool to verify consistency (cross-tool deviations within `pass_conditions.json` ACPF thresholds). | **Convergence is a positive finding, not a requirement.** If the tool converges, record it as a discriminating strength in solver robustness (Expressiveness narrative). If the tool fails to converge, record the failure mode but do not penalize -- MATPOWER itself cannot solve this case. The test is informational evidence, not a gate. If multiple tools converge, cross-tool consistency checks apply using the `acpf` thresholds in `pass_conditions.json`. | `data/fnm/reference/cleaned/fnm_main_island.mat` (cleaned case), `data/fnm/reference/acpf/summary_acpf.json` (MATPOWER failure analysis), `data/fnm/reference/pass_conditions.json` (cross-tool thresholds) |
...
**Note on G-FNM-3 and G-FNM-4 (power flow verification):** G-FNM-3 and G-FNM-4 load from the pre-cleaned MATPOWER case (`data/fnm/reference/cleaned/fnm_main_island.mat`), not the raw intermediate format. The cleaned case has all data fixes pre-applied (negative-X coercion, zero-impedance fixes, zero-RATE_A fixes, main island extraction, single-slack reduction) per `summary_cleaning.json`. This ensures all tools solve on identical input data -- cleaning is not part of the test. G-FNM-1 and G-FNM-2 still use the raw intermediate format (testing ingestion fidelity). G-FNM-3 and G-FNM-4 use the cleaned case (testing solver accuracy on known-good input).
...
- Results produced under v7 remain valid for Suites A-G. E-8 is additive. Mixed-version result sets (v7 for Suites A-G, v8 for E-8) are expected and do not require re-evaluation of existing results. Results produced under v6 (before cleaned case export and G-FNM-4 reframing) should be re-evaluated under v7+.
...
7. **FNM Ingestion (Suite G) — when FNM_PATH is set:** Suite G results provide additional evidence for Expressiveness and Extensibility grades, as specified in rubric v6 grading notes. The mapping is:
...
| v8 | 2026-03-09 | Maturity restructuring: added E-8 (reviewer/approval concentration), added weighting guidance (track record primary, sustainability risk secondary with bounded modifier). Version-awareness: protocol now requires research phase to identify installed version capabilities. Ingestion fidelity: strengthened post-ingestion verification guidance. Aligned with rubric v6. | GRC |
