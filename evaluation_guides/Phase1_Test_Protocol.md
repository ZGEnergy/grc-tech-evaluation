...
| LARGE | FNM Annual S01 | ~30,000 | PSS/E v31 via intermediate format | Data model fidelity — Suite G FNM ingestion tests (FNM_PATH-gated). |
...
| A-5 | SCUC | Solve 24-hour unit commitment as MILP with: min up/down times, startup costs, ramp rates, reserve requirements | yes | SMALL | Solves to feasibility (MIP gap ≤ 1% on TINY, ≤ 10% on SMALL). Commitment schedule extractable as a time-indexed binary matrix. Built-in constraint types vs. user-assembled noted. |
...
| C-6 | 20-scenario stochastic DCOPF (12hr, independent perturbations by resource type) | SMALL | HiGHS | Total time, per-scenario average, peak memory, price extraction |
...
| C-8 | SCOPF (N-1, 50 contingencies on SMALL, 500 on MEDIUM) | MEDIUM | HiGHS | Wall-clock time, peak memory, number of iterations (if screening), number of binding contingencies |
...
- ACPF reference solution: `data/fnm/reference/acpf/`
...
- **`protocol_version`** — the version of this protocol used (currently `"v4"`)
- For Suite G results, use `protocol_version: "v5"`. Existing Suite A-F results produced under v4 remain valid — Suite G is additive and does not alter any v4 test definitions. Mixed-version result sets (v4 for Suites A-F, v5 for Suite G) are expected and do not require re-evaluation of existing results.
...
7. **FNM Ingestion (Suite G) — when FNM_PATH is set:** Suite G results provide additional evidence for Expressiveness and Extensibility grades, as specified in rubric v4 grading notes. The mapping is:
   ...
   - **Grade impact:** Suite G results strengthen or weaken the evidence base for the grade assigned by Suites A-F, but do not independently determine the grade. A tool that passes all synthetic-network tests but fails FNM ingestion entirely receives a grade note documenting the gap. The A/B/C grade boundaries defined in the rubric are unchanged. See rubric v4 grading notes under Criterion 1 ("Note on FNM data model fidelity") and Criterion 2 ("Note on FNM supplemental CSV representability") for the authoritative grade impact specification.
...
| v5 | 2026-03-06 | FNM ingestion expansion: added Test Suite G (FNM Ingestion) with 5 test cases — G-FNM-1 (intermediate format ingestion gate), G-FNM-2 (field coverage audit vs. criticality matrix), G-FNM-3 (DCPF verification vs. reference), G-FNM-4 (ACPF verification vs. reference), G-FNM-5 (supplemental CSV representability). All Suite G tests gated by FNM_PATH. Added FNM_PATH gating rule to General Rules. Added LARGE reference network (FNM Annual S01, ~30K buses). Added Suite G to "From Test Results to Grades" with Expressiveness and Extensibility mapping. Added fnm_ingestion results directory. Mixed-version result sets (v4 + v5) explicitly permitted. Aligned with rubric v4 grading notes. | GRC |
