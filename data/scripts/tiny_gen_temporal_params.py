...
def compute_min_times(
    ...
    """
    if not is_flexible:
        min_up = max(template_min_up_hr, MIN_UP_DOWN_TIME_FLOOR_HR)
        min_down = max(template_min_down_hr, MIN_UP_DOWN_TIME_FLOOR_HR)
...
def assign_temporal_params(
    ...
    # Enforce cold >= warm >= hot ordering for startup costs.
    # RTS-GMLC data can violate this for nuclear (single-unit sample).
    cost_cold = template.startup_cost_cold_dollar
    cost_warm = min(template.startup_cost_warm_dollar, cost_cold)
    cost_hot = min(template.startup_cost_hot_dollar, cost_warm)
    ...
    return GenTemporalParams(
        ...
        startup_cost_cold_dollar=cost_cold,
        startup_cost_warm_dollar=cost_warm,
        startup_cost_hot_dollar=cost_hot,
