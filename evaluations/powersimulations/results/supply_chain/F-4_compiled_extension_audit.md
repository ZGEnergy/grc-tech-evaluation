...
| Component | Language | JLL Package | Version | Source Repo | License |
|-----------|----------|-------------|---------|-------------|---------|
...
| SCIP | C++ | SCIP_jll v1000.0.2+0 | 10.0.0 | [scipopt/scip](https://github.com/scipopt/scip) | Apache-2.0 |

### Numerical Libraries (linked by solvers or Julia runtime)

| Component | Language | JLL Package | Version | Source Available |
|-----------|----------|-------------|---------|-----------------|
| OpenBLAS | C/Fortran | OpenBLAS_jll v0.3.23+4 | 0.3.23 | Yes (GitHub) |
| MUMPS (sequential) | Fortran | MUMPS_seq_jll v500.800.100+0 | 5.8.1 | Yes (MUMPS consortium) |
| SPRAL | Fortran | SPRAL_jll v2025.5.20+0 | 2025.5.20 | Yes (GitHub) |
| ASL (AMPL Solver Library) | C | ASL_jll v0.1.3+0 | 0.1.3 | Yes (GitHub) |
| GMP | C | GMP_jll v6.2.1+6 | 6.2.1 | Yes (GNU) |
| METIS | C | METIS_jll v5.1.3+0 | 5.1.3 | Yes (Karypis Lab) |
| HDF5 | C | HDF5_jll v1.14.2+1 | 1.14.2 | Yes (HDF Group) |
| MKL | C/Fortran | MKL_jll v2024.2.0+0 | 2024.2.0 | No (Intel proprietary) |

### Other JLL Packages (52 total in dependency tree)

The full dependency tree includes 52 JLL packages. Beyond the solver and numerical
libraries listed above, these are system-level libraries (zlib, bzip2, OpenSSL,
SQLite, XML2, etc.) that are standard, widely audited components.
...
performance optimization for BLAS/LAPACK. The `MKL.jl` package in the dependency
tree can be replaced with `OpenBLAS_jll` (which is also present) by not loading
the MKL package. MKL is not required for any solver operation -- it accelerates
dense linear algebra in Julia's standard library.
