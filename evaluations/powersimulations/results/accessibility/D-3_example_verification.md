...
in a standard PSI installation** and is not listed as a dependency. **Zero official examples
run unmodified** in a standard PSI installation. The PowerFlows.jl tutorial (1 tutorial)
works with a minor MATPOWER file substitution. The how-to guides are reference patterns,
not self-contained runnable examples.
...
The tutorial gap compounds with other accessibility friction documented in observations:
- [api-friction A-3](../observations/api-friction-expressiveness-A-3_dcopf.md): Even when
  a user adapts a tutorial, single-snapshot OPF requires time series boilerplate not shown
  in any example
- [api-friction A-1](../observations/api-friction-expressiveness-A-1_dcpf.md): DC/AC power
  flow return type inconsistency means the one working tutorial pattern does not generalize
  cleanly to both power flow types
- [doc-gaps B-2](../observations/doc-gaps-extensibility-B-2_graph_access.md): Topology
  traversal from adjacency matrices has no example or documentation
