...
storage devices via `set_device_model!()`. The `src/devices_models/devices/` directory in
PSI v0.30.2 contains files for thermal, renewable, load, branch, HVDC, and regulation
devices but no storage file.
...
models (`OTHER_ERROR`). Single-period QP (A-3) works but 24-period QP does not. This appears
to be a HiGHS bug with larger QP models. Ipopt handles the same QP correctly. Used linear
costs with HiGHS instead.
...
All 24 hours have >= 2 binding branches. Consistently congested branches:
- `bus-1-bus-2` and `bus-2-bus-3` bind in all 24 hours
- `bus-16-bus-19` binds in hours 9-22 (peak and shoulder periods)
- `bus-6-bus-11` binds in hours 18-19 (peak)

**Condition 2: BESS Arbitrage (FAIL)**
- Average charge LMP: $36.61/MWh (24 charging hours)
- Average discharge LMP: $35.68/MWh (21 discharging hours)
- Discharge LMP is NOT greater than charge LMP

The BESS charges and discharges simultaneously in most hours because there is no mutual
exclusion constraint (would require binary variables, making it a MIP). With linear costs,
LMPs are non-unique at the margin, and the BESS operates as a net load (total charge 3,324
MWh > total discharge 2,906 MWh) rather than performing price arbitrage. The round-trip
efficiency loss (87.4%) means the optimizer uses the BESS as a "loss sink" to reduce
generation at expensive buses.
...
Total renewable generation: 9,663 MWh (approximately 7.6% of total system energy).

**LMP summary (selected hours):**

| Hour | Min $/MWh | Max $/MWh | Spread | Mean |
|------|-----------|-----------|--------|------|
| 1 | 5.00 | 43.89 | 38.89 | 23.74 |
| 4 (valley) | 5.00 | 42.48 | 37.48 | 22.52 |
| 12 | 5.00 | 46.65 | 41.65 | 31.35 |
| 18 (peak) | 5.00 | 46.65 | 41.65 | 32.60 |
| 24 | 5.00 | 43.89 | 38.89 | 23.77 |
...
- **Durability:** fragile — The JuMP-level BESS injection modifies PSI's internal constraint
  ...
  (likely v0.31+) may add native storage support, making this workaround unnecessary.
- **Grade impact:** Multi-period DCOPF itself works natively (24h horizon, hourly resolution,
  ...
  The BESS arbitrage condition fails due to linear costs producing non-unique LMPs at the
  margin, combined with simultaneous charge/discharge in the LP relaxation.

## Timing

- **Wall-clock:** 0.108 s (second run, after JIT warm-up; includes build + solve)
- **Timing source:** measured
- **Peak memory:** 1308.2 MB (Julia process RSS)
- **CPU cores used:** 1
...
@constraint(jm, bess_soc[end] == init_soc)
