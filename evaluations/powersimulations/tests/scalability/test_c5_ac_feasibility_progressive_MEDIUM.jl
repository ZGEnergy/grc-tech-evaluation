...
Network: MEDIUM (ACTIVSg 10k-bus, 12706 branches, 2485 generators)
...
function attempt_acpf(sys; label::String="", timeout_seconds::Float64=1800.0)
...
function run(network_file::String="/workspace/data/networks/case_ACTIVSg10k.m")
    ...
    try
        ...
        # Step 1: Load system
        println(stderr, "Loading MEDIUM system (10k bus)...")
        ...
        println(stderr, "System loaded: $n_buses buses, $n_branches branches, $n_gens generators")
        ...
        # DCPF returns Dict with period key "1"
        ...
        # Build lookup for fast warm-start application
        ...
        for (i, (relax, label)) in enumerate(zip(relaxation_levels, relaxation_labels))
            ...
            println(stderr, "Loading fresh system for relaxation $label...")
            sys_attempt = load_system(network_file)

            # Re-apply DCPF warm-start angles using lookup
            ...
            attempt = attempt_acpf(sys_attempt; label="relaxation $label", timeout_seconds=1800.0)
        ...
        if achieved_level !== nothing
            ...
            push!(
                results["errors"],
                "AC power flow infeasible on MEDIUM even with 20% thermal relaxation",
            )
        end
        ...
        push!(
            ...
            "Used Dict lookup for bus angles instead of nested loop (O(n) vs O(n²) for 10k buses).",
        )
