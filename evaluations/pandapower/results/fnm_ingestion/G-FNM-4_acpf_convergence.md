...
main island network. All four available NR algorithms (nr, iwamoto_nr,
fdbx, fdxb) and Gauss-Seidel (gs) were attempted; none converged.
...
3. Attempted ACPF at 0% relaxation with `pandapower.runpp(net,
   init="results")` to use DCPF angles as warm-start.
4. After failure at 0%, multiplied all branch thermal limits by 1.10
   and retried ACPF (10% relaxation).
5. After failure at 10%, multiplied limits by 1.20 and retried (20%
   relaxation).
6. All three relaxation levels failed to converge.

### Additional Algorithm Attempts

Beyond the standard Newton-Raphson (`nr`) with `init="results"`, the
following configurations were also tested:

| Algorithm | Max Iterations | Tolerance | Q-Lims | Init | Converged |
|-----------|---------------|-----------|--------|------|-----------|
| `nr` | 100 | 1e-6 | yes | results | No |
| `nr` | 200 | 1e-4 | no | results | No |
| `nr` | 200 | 1e-4 | no | flat | No |
| `iwamoto_nr` | 500 | 1e-4 | no | results | No |
| `fdbx` | 500 | 1e-4 | no | results | No |
| `fdxb` | 500 | 1e-4 | no | results | No |
| `gs` | 500 | 1e-4 | no | results | No |

The Iwamoto-Newton-Raphson algorithm showed characteristic behavior of a
numerically ill-conditioned system: the Iwamoto step-length multiplier
decayed from ~0.09 to values below 1e-24 over 500 iterations, indicating
the NR direction was correct but the system could not take meaningful
...
|--------|-------|
| DCPF converged | yes |
| DCPF solve time | 1.22 s |
| DCPF mean |VA| (all buses) | 209.5 deg |
| DCPF max |VA| | 536.9 deg |

The large DCPF voltage angles (max 536.9 degrees) indicate unusual
operating conditions in the network — angles exceeding 360 degrees
suggest either a topology anomaly or a localized sub-region with
extreme angle differences. This is consistent with the localized
data ingestion issues identified in G-FNM-3 (101 buses with 14-21
degree systematic deviations from reference).

### Progressive Relaxation Results

| Step | Relaxation | Converged | Wall-Clock (s) | Error |
|------|------------|-----------|----------------|-------|
| 2 | 0% | No | 1.94 | NR did not converge after 100 iterations |
| 3 | 10% | No | 0.34 | NR did not converge after 100 iterations |
| 4 | 20% | No | 0.34 | NR did not converge after 100 iterations |
...
4. **pandapower does not use Ipopt for ACPF.** Unlike the methodology
   specification's suggestion to use Ipopt, pandapower's `runpp()` uses
...
- **Wall-clock:** 7.65 s (total: load + DCPF + 3 ACPF attempts)
