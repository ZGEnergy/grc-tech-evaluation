...
element (8,576 vs 15,062 expected) and classifies branches by voltage level rather
than tap ratio, splitting the 33,840 MATPOWER branch records into 24,165 lines +
2,393 trafos + 7,282 impedances instead of the intermediate format's 24,117
branches + 9,723 transformers. Additionally, 55 extra sgen elements are created
from buses with negative active power demand.

## Context

During G-FNM-1 ingestion testing, pandapower loaded the 30,307-bus FNM network
via `from_ppc()`. The merged branch total (33,840) and bus count (30,307) match
