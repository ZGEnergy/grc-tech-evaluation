...
def run(
    ...
    """Execute AC feasibility progressive relaxation test on MEDIUM network."""
    results: dict = {
        "status": "fail",
    ...
    try:
        ...
        try:
            ...
            results["details"]["lightsim2grid_available"] = True
        except ImportError:
            results["details"]["lightsim2grid_available"] = False
        ...
        # =========================================================
        # STEP 2: ACPF at 0% relaxation (DC warm start, lightsim2grid)
        # =========================================================
        use_ls2g = results["details"]["lightsim2grid_available"]
        converged_0, time_0 = _try_acpf(
            net, init="dc", max_iteration=100, use_lightsim2grid=use_ls2g
        )
        ...
        if converged_0:
            ...
            converged_0_flat, time_0_flat = _try_acpf(
                net, init="flat", max_iteration=100, use_lightsim2grid=use_ls2g
            )
        ...
        if not step_0_result.get("converged"):
            ...
            converged_10, time_10 = _try_acpf(
                net, init="dc", max_iteration=100, use_lightsim2grid=use_ls2g
            )
            step_10_result: dict = {
                "relaxation_pct": 10,
            ...
            if converged_10:
                ...
                converged_10_flat, time_10_flat = _try_acpf(
                    net, init="flat", max_iteration=100, use_lightsim2grid=use_ls2g
                )
            ...
            if not step_10_result.get("converged"):
                ...
                converged_20, time_20 = _try_acpf(
                    net, init="dc", max_iteration=100, use_lightsim2grid=use_ls2g
                )
                step_20_result: dict = {
                    "relaxation_pct": 20,
                ...
                if converged_20:
                    ...
                    converged_20_flat, time_20_flat = _try_acpf(
                        net, init="flat", max_iteration=100, use_lightsim2grid=use_ls2g
                    )
