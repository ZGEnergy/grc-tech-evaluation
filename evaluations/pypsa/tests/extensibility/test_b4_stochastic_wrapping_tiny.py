"""B-4 (stochastic_wrapping) — 50-scenario stochastic DCPF on IEEE 39-bus (TINY).

Pass condition: Timeseries injectable via API, scenario loop without excessive overhead.
Generate correlated load/renewable perturbations, solve 24hr multi-period DCPF per scenario.
"""
...
def generate_correlated_scenarios(
    ...
    """Generate temporally correlated load scenarios.

    Uses a multivariate normal distribution with temporal correlation
    (AR(1) structure) to produce realistic load perturbations.
    """
    rng = np.random.default_rng(seed)
    n_loads = len(base_loads)

    # AR(1) temporal correlation matrix
    ...
    for s in range(n_scenarios):
        # Generate correlated temporal perturbations for each load
        perturbations = rng.multivariate_normal(np.zeros(n_hours), 0.05**2 * time_cov)
        # Apply same temporal pattern to all loads with slight per-load variation
...
def run() -> dict:
    """Execute B-4 stochastic wrapping test."""
    errors = []
    workarounds = []
    details = {}
    ...
    try:
            ...
            scenario_results.append(
                {
                    ...
                    "total_load_mw_range": [
                        round(float(load_profile.sum(axis=1).min()), 2),
                        round(float(load_profile.sum(axis=1).max()), 2),
                    ],
                }
            )
        ...
        details["per_scenario_avg_seconds"] = round(wall_clock / N_SCENARIOS, 6)
        ...
        details["api_method"] = (
            "Set n.set_snapshots(), assign n.loads_t.p_set per load, "
            "call n.lpf(). No config file rewriting needed."
        )
        details["timeseries_injection"] = "Programmatic via n.loads_t.p_set DataFrame"
        details["loc"] = 20

        assert len(scenario_results) == N_SCENARIOS, "Not all scenarios completed"
        status = "PASS"
