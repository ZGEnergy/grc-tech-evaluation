...
Pass condition: Tool supports distributed slack formulation. LMPs differ from
  single-slack results in a physically consistent manner (slack bus no longer has
  unique reference LMP).
Tool: PyPSA 1.1.2
...
Methodology:
  ...
  OPF via angle-sum-zero constraint is NOT achievable — the angle reference concept
  does not apply to PyPSA's DC OPF formulation.
  ...
  3. Attempt extra_functionality workaround for distributed slack in OPF context
...
# A-3 reference LMPs (from A-3_dcopf.md) — notable buses only
A3_LMP_SAMPLE = {
    "2": 10.0,
    "3": 763.3,
    "30": 10.0,
...
def run(network_file: str = DEFAULT_NETWORK, timeseries_dir: str | None = None) -> dict:
    ...
    try:
        ...
        # Identify slack bus (bus with v_ang closest to 0)
        v_ang_vals = n.buses_t.v_ang
        if len(v_ang_vals) > 0 and len(v_ang_vals.columns) > 0:
            slack_bus = v_ang_vals.iloc[0].abs().idxmin()
            results["details"]["slack_bus"] = slack_bus
            results["details"]["slack_bus_lmp"] = float(single_slack_lmps.get(slack_bus, 0))
        else:
            # v_ang not populated after DC OPF — find from sub_network
            results["details"]["slack_bus"] = "unknown"
        ...
        # Key finding: document whether Bus-v_ang exists in the OPF model
        ...
        # This demonstrates PyPSA's distributed slack capability in PF (not OPF) context
        ...
        try:
            ...
            # Set p_set to A-3 OPF dispatch values
            p_set_df = pd.DataFrame(single_slack_dispatch, index=[n2.snapshots[0]])
            ...
            # Read convergence from pf_result Dict (keys: n_iter, error, converged)
            ...
            if pf_converged:
                ...
                # Compare voltages to single-slack (regular pf without distribute_slack)
                n3 = load_network(network_file)
                setup_network(n3)
                p_set_df3 = pd.DataFrame(single_slack_dispatch, index=[n3.snapshots[0]])
                n3.generators_t.p_set = p_set_df3
        ...
        # --- Step 4: OPF distributed slack summary ---
        # Since Bus-v_ang doesn't exist in linopy model, custom angle-sum constraint impossible.
        ...
        # Pass: "Tool supports distributed slack formulation. LMPs differ from single-slack."
        # PyPSA v1.1.2 finding:
        # - Distributed slack IN PF context: YES (n.pf(distribute_slack=True))
        # - Distributed slack IN OPF context: NO — architecture does not expose angle vars
        # The pass condition is specifically about OPF, not just PF.
        # Since OPF distributed slack is NOT achievable via any API path, this is a FAIL
        # for the OPF-specific pass condition.
        # However, the PF-context distributed slack IS a meaningful capability.
