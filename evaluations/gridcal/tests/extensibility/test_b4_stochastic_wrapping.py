"""B-4: Stochastic Wrapping on IEEE 39-bus (TINY).
...
NOTE: Time-series OPF crashes on case39.m with TapPhaseControl error.
...
def run() -> dict:
    ...
    try:
        ...
        details["resource_classification"] = {
            i: {"name": gen_data[i]["name"], "type": resource_map[i]} for i in range(n_gen)
        }

        # ── Step 2: Generate correlated scenarios ──
        np.random.seed(42)

        # Correlation matrix per resource type (shared factor + individual noise)
        ...
        for s in range(N_SCENARIOS):
            # Correlated perturbation: shared factor per resource type
            type_factors = {}
        ...
        try:
            ...
            workarounds.append(
                {
                    "description": (
                        "Time-series OPF crashes on case39.m with TapPhaseControl error. "
                        "Using hour-by-hour snapshot OPF loop as workaround."
                    ),
                    "class": "fragile",
                    "reason": "Known GridCal bug on case39.m transformer handling.",
                }
            )
            ...
            scenario_results.append(
                {
                    ...
                    "total_hours": N_HOURS,
                    "dispatch_mean_mw": [round(float(x), 2) for x in hourly_dispatch.mean(axis=0)],
        ...
        if all_converged:
            if ts_opf_works:
                ...
                details["pass_rationale"] = (
                    "Time-series OPF works; 20 scenarios x 12 hours solved. "
                    "Prices and dispatch collected."
                )
            else:
                status = "qualified_pass"
                details["pass_rationale"] = (
                    f"20 scenarios x 12 hours = {total_solves} snapshot OPFs solved. "
                    "Prices and dispatch collected per scenario. "
                    "However, uses snapshot OPF loop (not multi-period) due to "
                    "TapPhaseControl crash. No inter-temporal constraints."
                )
        else:
            status = "fail"
            errors.append(
