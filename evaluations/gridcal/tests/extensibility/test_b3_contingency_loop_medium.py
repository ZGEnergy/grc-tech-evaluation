"""B-3: N-1 Contingency Loop on ACTIVSg10k (MEDIUM).

Dimension: extensibility
Network: MEDIUM (ACTIVSg 10k-bus)
Pass condition: Runs 50 N-1 DCPF contingencies in loop without re-parsing base model.
...
def run() -> dict:
    """Execute B-3 contingency loop test on MEDIUM network."""
    errors: list[str] = []
    ...
    try:
        ...
        base_loading = np.abs(base_results.loading.real) * 100
        details["baseline"] = {
            "converged": True,
        ...
        # ── Step 2: Select top-50 most loaded branches for contingencies ──
        loading_order = np.argsort(base_loading)[::-1]
        contingency_indices = loading_order[:N_CONTINGENCIES].tolist()
        details["contingency_selection"] = "Top 50 most loaded branches"
        details["contingency_count"] = N_CONTINGENCIES
        ...
        for ci, branch_idx in enumerate(contingency_indices):
            # Disable branch in-place
            branches[branch_idx].active = False
            ...
            # Re-enable branch
            branches[branch_idx].active = True

        t_loop = time.perf_counter() - t0_loop
        wall_clock = t_base + t_loop

        details["n1_loop"] = {
            "total_contingencies": N_CONTINGENCIES,
            "converged": N_CONTINGENCIES - non_converged_count,
            "non_converged": non_converged_count,
            "wall_clock_seconds": round(t_loop, 6),
            "per_contingency_avg_ms": round(t_loop / N_CONTINGENCIES * 1000, 4),
        ...
        # ── Step 4: Verify model integrity after loop ──
        ...
        all_ran = details["n1_loop"]["total_contingencies"] == N_CONTINGENCIES

        if model_ok and all_ran:
            status = "pass"
            details["pass_rationale"] = (
                f"{N_CONTINGENCIES} N-1 contingencies solved on 10k-bus network without "
                f"re-parsing model. branch.active toggle modifies in-place; model integrity "
                f"verified. Loop took {round(t_loop, 3)}s "
                f"({round(t_loop / N_CONTINGENCIES * 1000, 1)}ms/case)."
            )
        else:
            status = "fail"
            if not model_ok:
