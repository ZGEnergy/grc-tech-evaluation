"""
Test C-8: SCOPF (N-1, 50 contingencies) on SMALL.

Dimension: scalability
Network: SMALL (ACTIVSg 2000-bus)
Pass condition: SCOPF (N-1, 50 contingencies) solves on SMALL.
Tool: gridcal (VeraGridEngine) 5.6.28
Solver: HiGHS

Scales A-9 (SCOPF on TINY) to the SMALL network with 50 contingencies.
...
by loading percentage descending. This ensures contingencies are meaningful
(outage of heavily-loaded lines creates the most severe post-contingency flows).
"""
...
def run(
    ...
    """Execute C-8 SCOPF scale test on SMALL and return structured results."""
    results = {
        "status": "fail",
    ...
    try:
        ...
        if not results_base.converged:
            results["errors"].append("Base-case DCOPF did not converge on SMALL")
            return results
        ...
        if not converged:
            results["errors"].append("SCOPF did not converge on SMALL with 50 contingencies")
            return results
