"""
Test C-5: AC feasibility with progressive relaxation (0%, 10%, 20%) on MEDIUM.

Dimension: scalability
Network: MEDIUM (ACTIVSg 10000-bus)
Pass condition: AC Feasibility progressive relaxation on MEDIUM. Record relaxation
...
Scales A-2 (ACPF) and A-4 (AC feasibility) to MEDIUM. The test:
1. Solves DC OPF on the MEDIUM network
2. Fixes generator dispatch to the DC OPF solution
3. Attempts ACPF with progressive voltage relaxation assessment:
   - 0%: strict [0.95, 1.05] pu limits
   - 10%: relaxed [0.945, 1.055] pu
   - 20%: relaxed [0.940, 1.060] pu
4. Records which relaxation level achieves feasibility and violation counts
"""
...
def run(
    ...
    """Execute C-5 AC feasibility progressive relaxation test on MEDIUM."""
    results = {
        "status": "fail",
    ...
    try:
        ...
        # Try alternative solvers if still not converged
        alt_solvers_tried = {}
        if not converged:
            alt_solver_types = [
        ...
        if not converged:
            ...
            if direct_converged:
                ...
                results["workarounds"].append(
                    "ACPF with DCOPF-fixed dispatch did not converge on MEDIUM. "
        ...
        if not converged:
            results["errors"].append(
                "ACPF did not converge on MEDIUM "
                "(flat start, DC warm start, relaxed tol all failed)"
            )
        ...
        pass_checks = {
            "acpf_converges_on_medium": converged,
