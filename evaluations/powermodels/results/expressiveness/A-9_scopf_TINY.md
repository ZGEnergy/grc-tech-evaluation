...
`PowerModelsSecurityConstrained.jl` is not installed (listed in `research-extensions.md` but not in the project manifest). Proceeded with manual iterative Benders cutting plane using documented two-level API.

### Step 2: Full N-1 SCOPF LP (mechanism verification)

Before running the iterative algorithm, added all 4,140 N-1 security constraints (46 contingencies x ~90 monitored lines each x 2 bounds) at once to verify the API mechanism. This revealed the IEEE 39-bus network is not fully N-1 secure at original ratings (see Key Finding).
...
3. Compute LODF matrix from PTDF: `LODF_lk = (PTDF[l,f_k] - PTDF[l,t_k]) / (1 - PTDF[k,f_k] + PTDF[k,t_k])`
...
**Note on 70% derating:** Using A-3's 70% branch derating makes SCOPF infeasible even for individual contingencies (branches 35 and 38 form a near-series path with LODF approximately -1.0; N-1 outage of either forces flow exceeding the derated limit on the other regardless of redispatch). This is why the SCOPF test uses original (100%) ratings.
...
- **Why:** PMSC.jl is an ecosystem package listed in the extension registry but not installed. The manual implementation uses `instantiate_model`, `var(pm, :p)`, `@constraint(pm.model, ...)`, and `optimize_model!` -- all documented public API.
- **Durability:** stable -- All API functions used are documented and part of the stable public interface. The PTDF/LODF computation uses `calc_basic_ptdf_matrix` which is also public API (available since v0.13.2).
- **Grade impact:** B-level. The mechanism is correct and the workaround is clean. The network's N-1 infeasibility is a data limitation, not a tool limitation. The two-level API pattern is well-documented and stable.
...
# Security constraint addition via two-level API
pm = PowerModels.instantiate_model(deepcopy(data), PowerModels.DCPPowerModel,
                                    PowerModels.build_opf)
