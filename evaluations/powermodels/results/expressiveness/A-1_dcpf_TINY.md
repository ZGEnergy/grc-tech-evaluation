...
| Angle range | −0.219 to +0.109 rad (−12.6° to +6.3°) |
...
**Note on `termination_status`:** Returns a `Bool` (not a JuMP enum), which is undocumented in the official docs. Code must handle `result["termination_status"] == true` rather than checking for `"OPTIMAL"` or `"LOCALLY_SOLVED"`. This is an API inconsistency between `compute_dc_pf` (non-JuMP) and `solve_dc_opf` (JuMP-based).
