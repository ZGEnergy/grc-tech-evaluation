...
blocked_by: null
wall_clock_seconds: 7.55
timing_source: measured
peak_memory_mb: 2605.8
...
The full 12,706 x 10,000 dense PTDF matrix was computed successfully in a single call (2.37s). Phase-shifting transformer detection confirmed 5 branches in the raw network with nonzero SHIFT angles. `make_basic_network` absorbs these into the B-matrix construction, so the basic network has 0 phase-shifter rows and the standard `flow = PTDF * Pinj` formula produces exact results.

Flow accuracy was validated against reference DCPF flows: max absolute error = 2.18e-11 pu (numerical precision noise), far below the 1e-6 tolerance.
...
| Computation time (make_basic_network) | 1.16s |
| Computation time (calc_basic_ptdf_matrix) | 2.37s |
| DCPF reference time | 0.12s |
| Total wall-clock | 7.55s |
| Matrix size | 969.4 MB (Float64) |
| Peak process RSS | 2,605.8 MB |
| Memory delta during PTDF computation | 1,713.3 MB |
...
| PTDF x Pinj vs DCPF max error | 2.18e-11 pu |
| PTDF x Pinj vs DCPF mean error | 9.93e-14 pu |
...
- C-9: make_basic_network 1.16s, calc_basic_ptdf_matrix 2.37s, total 7.55s
...
- **Wall-clock:** 7.55s (includes network parse, make_basic_network, PTDF computation, DCPF reference, validation)
- **PTDF computation only:** 2.37s
- **make_basic_network:** 1.16s
- **DCPF reference:** 0.12s
- **Timing source:** measured (`time()` in Julia, post-JIT warm-up)
- **Peak memory:** 2,605.8 MB RSS
- **Memory delta during PTDF:** 1,713.3 MB (the 969.4 MB matrix plus intermediate allocation)
- **CPU cores used:** 1
...
# 12706 x 10000 dense matrix, 969.4 MB, 2.37s (warm JIT)
