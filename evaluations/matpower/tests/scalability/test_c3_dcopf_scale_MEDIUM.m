%% Test C-3: DC OPF on MEDIUM (ACTIVSg 10k) with MIPS and GLPK
%%
%% Dimension: scalability
%% Network: MEDIUM (ACTIVSg 10k)
%% Pass condition: Completes DC OPF on MEDIUM with multiple solvers.
%% Tool: MATPOWER 8.1
%% Solvers: MIPS, GLPK (HiGHS unavailable in Octave)
...
try
    ...
    if results_mips.success
        ...
        %% Max branch loading check (soft constraint detection)
        branch_pf = abs(results_mips.branch(:, PF));
        rate_a = mpc.branch(:, RATE_A);
        rated_branches = rate_a > 0;
        if any(rated_branches)
            loading_pct = branch_pf(rated_branches) ./ rate_a(rated_branches) * 100;
            max_loading = max(loading_pct);
            fprintf('  Max branch loading: %.4f%%\n', max_loading);
            if max_loading > 100.01  % 100% + 0.01% tolerance
                fprintf('  WARNING: Soft constraint detected -- max loading > 100%%\n');
            else
                fprintf('  Hard constraint enforcement confirmed\n');
            end
        end
    ...
    %% Build full PWL gencost
    mpc_pwl.gencost = zeros(ng_local, ncols_pwl);
    if any(has_range)
    ...
    if results_glpk.success
        ...
        binding_g = (mu_sf_g > 1e-6) | (mu_st_g > 1e-6);
        fprintf('  Binding branches: %d / %d\n', sum(binding_g), nl);

        %% Max branch loading check
        branch_pf_g = abs(results_glpk.branch(:, PF));
        if any(rated_branches)
            loading_pct_g = branch_pf_g(rated_branches) ./ rate_a(rated_branches) * 100;
            max_loading_g = max(loading_pct_g);
            fprintf('  Max branch loading: %.4f%%\n', max_loading_g);
        end
