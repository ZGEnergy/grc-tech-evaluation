%% Test C-3: DC OPF on SMALL (ACTIVSg 2000-bus) with MIPS and GLPK
%%
%% Dimension: scalability
%% Network: SMALL (ACTIVSg 2000-bus)
%% Pass condition: Completes DC OPF on SMALL with multiple solvers.
%% Tool: MATPOWER 8.1
...
try
    ...
    if results_mips.success
        ...
        fprintf('MIPS: FAILED\n');
        errors{end + 1} = 'MIPS failed on SMALL DC OPF';
    end
    ...
    %% GLPK only handles LP, not QP. ACTIVSg2000 has polynomial (quadratic) costs.
    %% Convert to piecewise-linear approximation. poly2pwl fails on gens with
    %% Pmin==Pmax (117 such generators in ACTIVSg2000), so we handle them manually.
    mpc_pwl = mpc;
    pmin_g = mpc.gen(:, PMIN);
    pmax_g = mpc.gen(:, PMAX);
    ng_local = size(mpc.gen, 1);

    %% For gens with range, use poly2pwl with 10 segments
    %% For gens with no range (Pmin==Pmax), create a 2-point PWL
    ...
    if results_glpk.success
        ...
        fprintf('GLPK: FAILED\n');
        errors{end + 1} = 'GLPK failed on SMALL DC OPF';
    end
