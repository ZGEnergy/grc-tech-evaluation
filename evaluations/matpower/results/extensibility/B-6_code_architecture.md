...
**File inventory (legacy DCPF path):**

| File | Lines | Role |
|------|-------|------|
| `lib/rundcpf.m` | 74 | Entry point, sets DC model flag, delegates to `runpf` |
| `lib/runpf.m` | 520 | Main PF dispatcher (AC/DC branching, Q-limit enforcement) |
| `lib/dcpf.m` | 46 | Core solver: sparse linear system solve `B\P` |
| `lib/makeBdc.m` | 86 | Build B-matrix with tap ratio and phase shift corrections |
| `lib/bustypes.m` | 48 | Classify bus types from bus/gen data |
| `lib/ext2int.m` / `lib/int2ext.m` | ~200 | Bus renumbering and data ordering |
...
**`dcpf.m` is the core solver at 46 lines.** The entire DC power flow computation is a single sparse matrix backslash operation:
```matlab
Va([pv; pq]) = B([pv; pq], [pv; pq]) \ ...
                (Pbus([pv; pq]) - B([pv; pq], ref) * Va0(ref));
```

**`makeBdc.m` implements the full B-matrix** incorporating tap ratios and phase shift angles (86 lines). Returns four outputs: `Bbus`, `Bf`, `Pbusinj`, `Pfinj` -- cleanly separating the bus injection matrix from the branch flow matrix and their phase-shift correction terms.
...
**Abstraction layers:** 4 distinct layers with clean boundaries.

| Layer | Base Class | DC Concrete Class | Responsibility |
|-------|-----------|-------------------|----------------|
| Data Model | `mp.data_model` (763 lines) | `mp.data_model` | Data parsing, validation, element tables |
| Network Model | `mp.net_model` | `mp.net_model_dc` (111 lines) | Port-injection parameters (B, K, p matrices) |
| Mathematical Model | `mp.math_model` | `mp.math_model_pf_dc` (64 lines) | Constraint formulation, solver interface |
| Task | `mp.task` | `mp.task_pf` (221 lines) | Orchestration, iteration control |
...
- **Legacy:** Functions documented via MATLAB-style help blocks (docstring at top of each file). Column indices documented in `idx_bus.m`, `idx_gen.m`, `idx_brch.m`, `idx_cost.m`. The MATPOWER User's Manual (240+ pages) covers the legacy API comprehensively.
...
### 5. Solver Interface Architecture
...
Each solver has its own options-conversion function (e.g., `glpk_options.m`, `gurobi_options.m`) and feature-detection function (e.g., `have_feature_glpk.m`). Solver swap is a single parameter change with no reformulation required.
...
5. **Full B-matrix:** `makeBdc.m` incorporates tap ratios and phase shift angles, producing four distinct output matrices with clear mathematical documentation.
...
4. **Flat data model (legacy):** The legacy `mpc` struct uses numeric matrices with integer column indices. While fast and transparent, it provides no type safety or column-name validation. Typos in column indices produce silent wrong results.
...
**Recorded metrics:**
- Abstraction layers: 3 (legacy) / 4 (MP-Core)
- Separation quality: Partial (legacy) / Clean (MP-Core)
- Documentation quality: Comprehensive (User's Manual, Developer Manual, inline docstrings, Extension API docs)
